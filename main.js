...
console.log(chalk.bold.blue(`<<<=======>>> Welcome to my Todo-List Application <<<=======>>>`));
console.log(chalk.bold.yellow(`\t <<<======= Syeda Maha Zaidi =======>>> \n`));
let main = async () => {
    while (condition) {
    ...
    let task_index = await inquirer.prompt([
        {
            name: "index",
            type: "number",
            message: chalk.green.italic("Enter the 'index No.' of the task you want to delete:"),
        },
    ]);
...
let update_task = async () => {
    ...
    let updated_task_index = await inquirer.prompt([
        {
            name: "index",
            type: "number",
            message: chalk.green.italic("Enter the 'index No.' of the task you want to update:"),
        },
        {
            name: "new_task",
            type: "input",
    ...
    console.log(`\nTask at index no ${updated_task_index.index - 1} updated successfully (for updated list check option: "View Todo-List")`);
};
main();
